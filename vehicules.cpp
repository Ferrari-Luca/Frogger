...
void SlidingObject::changeIsDeadly() { isDeadly = !isDeadly; }
...
void Nenuphar::draw() {
    physicalBorder.draw();
    if (isDeadly) {
        fl_draw_box(FL_FLAT_BOX, physicalBorder.getCenter().x - frogWidth / 2.0,
                    physicalBorder.getCenter().y - frogHeight / 2.0,
